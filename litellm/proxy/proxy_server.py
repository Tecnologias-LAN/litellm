...
async def proxy_startup_event(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    # MODIFICACION Tecnologias-LAN: mostrar el estado de Enterprise al arrancar
    if premium_user:
        verbose_proxy_logger.info(
            "\n"
            + "=" * 60
            + "\nLITELLM ENTERPRISE MODE: ENABLED\n"
            "   All Enterprise features are activated\n"
            "   ENTERPRISE_ALWAYS_ON=True (litellm/proxy/auth/litellm_license.py)\n"
            + "=" * 60
        )
    else:
        verbose_proxy_logger.info(
